...
2. Show Shafi that table exactly as printed (title, description, depends_on,
   sequence, labels for each row) and ask him to explicitly confirm before
   anything is created on GitHub. If the draft is empty, tell him there's
   nothing to exit with and stop — do not proceed to step 3.
